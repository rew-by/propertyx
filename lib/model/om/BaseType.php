...
/**
 * Base class that represents a row from the 'type' table.
 *
 * 
 *
...
abstract class BaseType extends BaseObject  implements Persistent {
  ...
  const PEER = 'TypePeer';
	...
	/**
	 * Initializes internal state of BaseType object.
	 * @see        applyDefaults()
	 */
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 2; // 2 = TypePeer::NUM_COLUMNS - TypePeer::NUM_LAZY_LOAD_COLUMNS).
    ...
    foreach (sfMixer::getCallables('BaseType:delete:pre') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseType:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseType:save:pre') as $callable)
    {
      ...
      if (is_int($affectedRows))
      {
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BaseType:save:post') as $callable)
    {
	 ...
	 * Otherwise if this Type has previously been saved, it will retrieve
	 * related Propertys from storage. If this Type is new, it will return
		...
		if ($this->collPropertys === null) {
			if ($this->isNew()) {
			   $this->collPropertys = array();
				...
				$criteria->add(PropertyPeer::TYPE_ID, $this->id);
  ...
  {
    if (!$callable = sfMixer::getCallable('BaseType:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BaseType::%s', $method));
    }
