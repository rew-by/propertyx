...
abstract class BaseOfferPeer {
	...
	const CLASS_DEFAULT = 'lib.model.Offer';
	...
	public static function getMapBuilder()
	{
		if (self::$mapBuilder === null) {
			self::$mapBuilder = new OfferMapBuilder();
		}
    ...
    foreach (sfMixer::getCallables('BaseOfferPeer:doCount:doCount') as $callable)
    {
      call_user_func($callable, 'BaseOfferPeer', $criteria, $con);
    }
    ...
    foreach (sfMixer::getCallables('BaseOfferPeer:doSelectStmt:doSelectStmt') as $callable)
    {
      call_user_func($callable, 'BaseOfferPeer', $criteria, $con);
    }
  ...
  static public function getUniqueColumnNames()
  {
	...
	{
		return OfferPeer::CLASS_DEFAULT;
	}

	/**
	 * Method perform an INSERT on the database, given a Offer or Criteria object.
	 *
    ...
    foreach (sfMixer::getCallables('BaseOfferPeer:doInsert:pre') as $callable)
    {
      $ret = call_user_func($callable, 'BaseOfferPeer', $values, $con);
      if (false !== $ret)
      {
        return $ret;
      }
    ...
    foreach (sfMixer::getCallables('BaseOfferPeer:doInsert:post') as $callable)
    {
      call_user_func($callable, 'BaseOfferPeer', $values, $con, $pk);
    }
	...
	/**
	 * Method perform an UPDATE on the database, given a Offer or Criteria object.
	 *
    ...
    foreach (sfMixer::getCallables('BaseOfferPeer:doUpdate:pre') as $callable)
    {
      $ret = call_user_func($callable, 'BaseOfferPeer', $values, $con);
      if (false !== $ret)
      {
		...
		if ($values instanceof Criteria) {
			...
			$selectCriteria->add(OfferPeer::ID, $criteria->remove(OfferPeer::ID), $comparison);
    ...
    foreach (sfMixer::getCallables('BaseOfferPeer:doUpdate:post') as $callable)
    {
      call_user_func($callable, 'BaseOfferPeer', $values, $con, $ret);
    }
	...
	/**
	 * Method to DELETE all rows from the offer table.
	 *
	 * @return     int The number of affected rows (if supported by underlying database driver).
	 */
	public static function doDeleteAll($con = null)
	{
		...
		try {
			...
			$affectedRows += BasePeer::doDeleteAll(OfferPeer::TABLE_NAME, $con);
			$con->commit();
			return $affectedRows;
		} catch (PropelException $e) {
			$con->rollBack();
	...
	/**
	 * Method perform a DELETE on the database, given a Offer or Criteria object OR a primary key value.
	 *
	 ...
	 */
	 public static function doDelete($values, PropelPDO $con = null)
	 {
		...
		} elseif ($values instanceof Offer) {
		...
		try {
			...
			// for more than one table or we could emulating ON DELETE CASCADE, etc.
			$con->beginTransaction();
			$affectedRows += OfferPeer::doOnDeleteCascade($criteria, $con);
	...
	protected static function doOnDeleteCascade(Criteria $criteria, PropelPDO $con)
	{
		...
		foreach ($objects as $obj) {
			...
			$c->add(PropertyPeer::OFFER_ID, $obj->getId());
	 ...
	 */
	public static function doValidate(Offer $obj, $cols = null)
	{
		$columns = array();
		...
		$res =  BasePeer::doValidate(OfferPeer::DATABASE_NAME, OfferPeer::TABLE_NAME, $columns);
    if ($res !== true) {
        $request = sfContext::getInstance()->getRequest();
        foreach ($res as $failed) {
            $col = OfferPeer::translateFieldname($failed->getColumn(), BasePeer::TYPE_COLNAME, BasePeer::TYPE_PHPNAME);
        }
    }
...
//
// NOTE: This static code cannot call methods on the OfferPeer class, because it is not defined yet.
// If you need to use overridden methods, you can add this code to the bottom of the OfferPeer class:
//
// Propel::getDatabaseMap(OfferPeer::DATABASE_NAME)->addTableBuilder(OfferPeer::TABLE_NAME, OfferPeer::getMapBuilder());
//
// Doing so will effectively overwrite the registration below.

Propel::getDatabaseMap(BaseOfferPeer::DATABASE_NAME)->addTableBuilder(BaseOfferPeer::TABLE_NAME, BaseOfferPeer::getMapBuilder());
