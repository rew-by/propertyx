...
abstract class BaseOffer extends BaseObject  implements Persistent {
  ...
  const PEER = 'OfferPeer';
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 2; // 2 = OfferPeer::NUM_COLUMNS - OfferPeer::NUM_LAZY_LOAD_COLUMNS).
    ...
    foreach (sfMixer::getCallables('BaseOffer:delete:pre') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseOffer:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseOffer:save:pre') as $callable)
    {
      ...
      if (is_int($affectedRows))
      {
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BaseOffer:save:post') as $callable)
    {
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = OfferPeer::doInsert($this, $con);
					...
					$affectedRows += OfferPeer::doUpdate($this, $con);
				}
	 ...
	 * Otherwise if this Offer has previously been saved, it will retrieve
	 * related Propertys from storage. If this Offer is new, it will return
		...
		if ($this->collPropertys === null) {
			if ($this->isNew()) {
			   $this->collPropertys = array();
				...
				$criteria->add(PropertyPeer::OFFER_ID, $this->id);
  ...
  {
    if (!$callable = sfMixer::getCallable('BaseOffer:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BaseOffer::%s', $method));
    }
